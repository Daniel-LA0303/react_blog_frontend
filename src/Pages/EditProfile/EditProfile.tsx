...
const EditProfile = () => {
  ...
  */
  ...
  const [inputSkill, setInputSkill] = useState<any>("");
  ...
  const store = useSelector((state: any) => state.userResponse);
  const updateUserRedux = (userId: any, editUserData: any, route: any) => dispatch(editUserAction(userId, editUserData, route));
  ...
  const handleRemoveSkill = (skillToRemove: any) => {
    setSkills(skills.filter((s: any) => s !== skillToRemove));
  };
  ...
  const handleSubmit = async (e: any) => {
    ...
    const skillsArray = skills.map((s: any) => s.trim());
