 ...
 */
   ...
   * States
   */
  const [isFollow, setIsFollow] = useState<boolean>(false); // -> to paint follow
  const [posts, setPosts] = useState<any>([]); // -> user's posts
  const [user, setUser] = useState<any>({}); //-> user view
              ...
              <div className={`overflow-hidden rounded-lg shadow ${globalData.themeGlobal ? 'bgt-light' : 'bgt-dark text-white'}`}>
                <div className="bg-gradient-to-br from-[var(--primary-100)] to-white p-6">
                  ...
                  <div className="w-full flex mt-2 justify-center sm:justify-end items-center">
                    {user._id === userAuth.userId || Object.keys(userAuth).length === 0 ? null : (
                    ...
                    <div className={`overflow-hidden rounded-lg shadow ${globalData.themeGlobal ? 'bgt-light' : 'bgt-dark text-white'}`}>
                      <div className="p-6">
                        ...
                        <div className="mt-4 flex flex-wrap gap-2">
                          {user?.info?.skills?.map((skill: any, i: any) => (
                            <span
                              key={i}
