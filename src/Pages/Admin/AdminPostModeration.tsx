...
import AnimatedRowPost from '../../components/Admin/AdminPostPanel.tsx/AnimatedRowPost'
import PostDetailDialog from '../../components/Admin/AdminPostPanel.tsx/PostDetailDialog'
...
const AdminPostModeration = () => {
  ...
  const [preview, setPreview] = useState<AdminPost | null>(null); // to show modal post info
  ...
  const fetchPosts = useCallback(async () => {
    setLoading(true)
    try {
      const { data } = await clientAuthAxios.get('/reports/posts-report', {
        params: {
          page: page + 1,
          limit: rowsPerPage,
  ...
  // key = status to change
  // postId = post toupdate
  const handleAction = async (key: string, postId: string) => {
      ...
      <main className="max-w-screen-xl mx-auto px-4 py-10 sm:px-6 lg:px-10 space-y-7">
        ...
        >
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
            ...
            {(['all', 'Published', 'Hidden', 'Deleted', 'Banned', 'Deleted by Admin', 'Hidden by Admin'] as (PostStatus | 'all')[]).map(s => (
              <Pill
                key={s}
