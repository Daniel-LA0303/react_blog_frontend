...
const AI_TOOLS = [
    ...
    { key: 'customPromt', label: 'Custom', icon: 'ti-tag', locked: false },
]
...
const PanelContent = ({
    ...
    dark: boolean
    onToolClick: (key: string, customPrompt?: string) => void
    userPlan?: 'FREE' | 'PRO' | 'PREMIUM'
    onClose?: () => void
}) => {
    ...
    const handleCustomSend = () => {
        ...
        onToolClick('custom', customPrompt.trim())
        //setCustomPrompt('')
    }
        ...
        <div className="flex flex-col gap-4">
            ...
            <div>
                ...
                <textarea
                    ...
                    rows={4}
                    className={`w-full px-3 py-2 rounded-xl border text-sm outline-none transition-colors duration-150 resize-none max-h-36 overflow-y-auto chat-scroll-dark
                        ${dark
                            ? 'bg-[#18181b] border-gray-800 text-gray-200 placeholder-gray-600 focus:border-gray-600'
...
const AIToolsPanel = ({ onToolClick, userPlan = 'FREE' }: AIToolsPanelProps) => {
    ...
    if (!isMobile) {
        ...
        ${dark
                ...
                <PanelContent
                    dark={dark}
                    onToolClick={(key, customPrompt) => {
                        onToolClick(key, customPrompt)
                        setTimeout(() => setBottomSheetOpen(false), 50) // ← let parent state settle first
                    }}
                    userPlan={userPlan}
