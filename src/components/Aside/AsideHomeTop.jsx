      ...
      <div className="flex flex-col gap-3">
        {topInfo?.users?.map(user => (
          <Link
            key={user._id}
            to={`/profile/${user._id}`}
            className={`${globalData.themeGlobal ? 'bgt-light' : 'bgt-dark text-white'} flex items-center px-4 py-3 hover:bg-slate-600 hover:text-white rounded-xl shadow`}
          >
            <div
      ...
      <div className="flex flex-col gap-3">
        {topInfo?.categories?.map(cat => (
          <Link
            ...
            key={cat._id}
            className={`${globalData.themeGlobal ? 'bgt-light' : 'bgt-dark text-white'} hover:bg-slate-600 hover:text-white w-full max-w-md flex flex-col rounded-xl shadow-lg px-3 py-4`}
          >
